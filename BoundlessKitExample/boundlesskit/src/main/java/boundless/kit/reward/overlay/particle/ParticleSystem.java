...
import boundless.kit.reward.overlay.particle.initializers.AccelerationInitializer;
import boundless.kit.reward.overlay.particle.initializers.LifetimeInitializer;
import boundless.kit.reward.overlay.particle.initializers.ParticleInitializer;
import boundless.kit.reward.overlay.particle.initializers.RotationInitializer;
import boundless.kit.reward.overlay.particle.initializers.RotationSpeedInitializer;
import boundless.kit.reward.overlay.particle.initializers.ScaleInitializer;
import boundless.kit.reward.overlay.particle.initializers.SpeedByComponentsInitializer;
import boundless.kit.reward.overlay.particle.initializers.SpeedModuleAndRangeInitializer;
import boundless.kit.reward.overlay.particle.modifiers.AlphaModifier;
import boundless.kit.reward.overlay.particle.modifiers.ParticleModifier;
		...
		for (int i=0; i<particleTemplates.size(); i++) {
			...
			if (template.drawable instanceof AnimationDrawable) {
				...
				if (template instanceof BlurredDrawableParticleTemplate) {
					...
					bitmap = blurBitmap(parentView.getContext(), bitmap, blurredTemplate.scale, blurredTemplate.radius);
				}
