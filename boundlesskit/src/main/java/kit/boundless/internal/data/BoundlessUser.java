...
public class BoundlessUser extends ContextWrapper {
    ...
    private BoundlessUser(Context base) {
        ...
        if (saved == null) {
            saved = new UserIdentityContract(
                    0,
                    null,
    ...
    public void update() {
        ...
        if (TextUtils.equals(saved.internalId, internalId)
                && TextUtils.equals(saved.externalId, externalId)
        ...
        BoundlessKit.debugLog("BoundlessUser", "Updating user to internalId:" + internalId + " externalId:" + externalId + " experimentGroup:" + experimentGroup);
